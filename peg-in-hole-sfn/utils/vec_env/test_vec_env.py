...
def test_vec_frame_stack():
    num_envs = 1
    def make_fn(seed):
        """
        Get an environment constructor with a seed.
        """
        return lambda: SimpleEnv(seed, shape=(6,1), dtype='uint8')
    fns = [make_fn(i) for i in range(num_envs)]
    env1 = SubprocVecEnv(fns)
    env2 = VecFrameStack(SubprocVecEnv(fns), nstack=3)
