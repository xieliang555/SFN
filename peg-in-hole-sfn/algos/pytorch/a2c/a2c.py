...
import gym
import time
import algos.pytorch.a2c.core as core
...
def a2c(venv, nenv, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        train_v_iters=80, lam=0.97, logger_kwargs=dict(), save_freq=10):
    """
    ...
    ac = actor_critic(venv.observation_space, venv.action_space, **ac_kwargs)
    ...
    def update():
        ...
        data = {'obs':obs, 'act':act, 'adv':adv, 'ret':ret, 'obs_next':obs_next}
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            if t == local_steps_per_epoch-1:
                _, last_val, _ = ac.step(torch.as_tensor(o, dtype=torch.float32))
                ...
                _, last_val, _ = ac.step(torch.as_tensor(ob_next, dtype=torch.float32))
                idx = np.where(d)[0]
