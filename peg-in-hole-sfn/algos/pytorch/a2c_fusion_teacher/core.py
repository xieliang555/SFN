...
class A2C(nn.Module):
    """docstring for A2C"""
    def __init__(self, obs_ft_dim, obs_img_dim, act_dim):
    ...
    # training network
    def forward(self, obs_ft, obs_img, act, obs_ft_adver, obs_img_adver):
        fea, fea_img = self.encoder(obs_ft, obs_img)
        _, fea_img_adver = self.encoder(obs_ft_adver, obs_img_adver)
        logits = self.fc_pi(fea)
        pi = Categorical(logits=logits)
        logp = pi.log_prob(act)
        v = self.fc_v(fea).squeeze(-1)
        return logp, v, fea_img, fea_img_adver

    # collecting data
    def step(self, obs_ft, obs_img):
        with torch.no_grad():
            fea, _ = self.encoder(obs_ft, obs_img)
            logits = self.fc_pi(fea)
            pi = Categorical(logits=logits)
            a = pi.sample()
            v = self.fc_v(fea).squeeze(-1) 
